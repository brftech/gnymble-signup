...
export function validateAndFormatEIN(ein: string): {
  isValid: boolean;
  formatted: string;
  error?: string;
} {
...
export function validateAndFormatState(state: string): {
  isValid: boolean;
  formatted: string;
  error?: string;
} {
  ...
  if (state.length === 2) {
    const isValidCode = Object.values(STATE_MAPPING).includes(
      state.toUpperCase()
    );
    if (isValidCode) {
      return {
...
export function validateAndFormatWebsite(website: string): {
  isValid: boolean;
  formatted: string;
  error?: string;
} {
