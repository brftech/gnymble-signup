...
-- Run this in Supabase SQL Editor to fix the webhook issues
...
AND tablename IN ('companies', 'brands', 'campaigns', 'phone_numbers', 'campaign_phone_assignments', 'onboarding_submissions', 'customers', 'subscriptions', 'customer_access', 'user_roles', 'profiles')
ORDER BY tablename;
...
-- Step 5: Test a simple insert to verify RLS is disabled
-- This should work without any RLS errors
INSERT INTO public.customers (email, company_name, primary_platform, customer_type, status) 
VALUES ('test@example.com', 'Test Company', 'gnymble', 'classic', 'active')
