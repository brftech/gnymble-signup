// Script to create the missing subscription and customer access records (fixed version)
...
async function createMissingRecordsFixed() {
  try {
    console.log("🔧 Creating Missing Records (Fixed Version)...\n");
    ...
    // Create subscription record (without updated_at)
    ...
    // Create customer access record (without updated_at)
    ...
    const accessData = {
      ...
      created_at: new Date().toISOString()
      // Removed updated_at since the column might not exist
    };
...
// Run the creation
createMissingRecordsFixed().catch(console.error); 